    ...
    .exec((err, retrievedUserDetails)=>{
      if(err){
         logger.error(err.message,'userController: signUp()/createUser()',10);
          reject(handleError('Error while creating user.',500,'createUser',err))
        ...
        newUser.save((err,newUser)=>{
           if(err){
            logger.error(err.message,'userController: signUp()/createUser()',10);
             reject(response.generate(true,'Error while saving new user.',500,null));
           }else{ 
         ...
         logger.info('Email already exists.','userController: signUp()/createUser()',7);
         reject(response.generate(true,'Email already exist.',500,null)); 
      }else if(retrievedUserDetails.userName === req.body.userName){
         logger.info('Username already exists.','userController: signUp()/createUser()',7);
   ...
   return new Promise((resolve,reject)=>{
      User.findOne({userId:req.params.userId}).exec((err, retrievedUserDetails)=>{
         if(err){
            ...
            logger.info('User not found.','userController: verifyUserEmail()/verifyEmail()',7);
            reject(response.generate(true,'User not found.',400,null));
         }else if(retrievedUserDetails.isVerified){
            logger.info('User is already verified.','userController: verifyUserEmail()/verifyEmail()',7);
            reject(response.generate(true,'Account is already verified.',400,null));
         }else{
            retrievedUserDetails.isVerified = true;
            retrievedUserDetails.save((err, result)=>{
               if(err){
                  logger.error(err.message,'userController: verifyUserEmail()/verifyEmail()',10);
                  reject(response.generate(true,'Failed to verifying user email.',500,null));
               }else{ 
...
let findUserByUserName = (req)=>{
   return new Promise((resolve,reject)=>{
      (req.body.email)? userName = {'email':req.body.email}: userName = {'userName':req.body.userName} ;
      User.findOne(userName)
      .select('-_id -__v').exec((err, retrievedUserDetails)=>{
        if(err){
            logger.error(err.message,'userController: login()/findUserByUserName()',10);
            reject(response.generate(true,'Error while finding user.',500,null));
...
let validatePassword = (req,userDetails)=>{
   return new Promise((resolve, reject)=>{ 
      passwordLib.comparePassword(req.body.password, userDetails.password, (err,isMatched)=>{
         if(err){
            logger.error(err.message,'userController: logIn()/validatePassword()',10);
            reject(response.generate(true,'Failed to compare password.',500,null));
            ...
            logger.info('Wrong password.','userController: logIn()/validatePassword()',7);
   ...
   return new Promise((resolve,reject)=>{
     token.generateToken(userDetails,(err, tokenDetails)=>{
         if(err){
            ...
            reject(response.generate(true,'Error while generate token.',500,null));
...
let saveToken = (tokenDetails)=>{
   return new Promise((resolve, reject)=>{
         ...
         if(check.isEmpty(retrievedTokenDetails)){
            ...
            newAuthToken.save((err, newTokenDetails)=>{
             ...
             retrievedTokenDetails.save((err, newTokenDetails)=>{
                if(err){
                  logger.error(err.message,'userController: saveToken()',10);
                  reject(response.generate(true,'Failed to save token.',404,null));
       ...
       .catch((err)=>{ reject(handleError('Error while saving token.',500,'saveToken',err)) });
    });
}
   ...
   return new Promise((resolve, reject)=>{
      Auth.remove({userId: req.user.userId}).exec((err, result)=>{
         if(err){
            ...
            logger.info('User already logged out.','userController:  deleteToken()',7);
   ...
   return new Promise((resolve, reject)=>{
      ...
      .exec((err, usersDetails)=>{
          if(err){
            ...
            reject(response.generate(true,'Failed to find all users.',500,null));
            
          }else if(check.isEmpty(usersDetails)){
            logger.info('Users details not found.','userController:  getAll()',7);
            reject(response.generate(true,'User already logged out.',404,null));
         }else{ resolve(usersDetails); }
      })
...
let getUser = (req)=>{
    return new Promise((resolve, reject)=>{
      User.findOne({ $and: [{ userId: req.params.userId },{ isVerified:true } ]})
      ...
      .exec((err, retrievedUserDetails)=>{
         ...
         }else{ resolve(retrievedUserDetails); }
      })
   });
}
      ...
      User.updateOne( { 'userId':userDetails.userId }, updateQuery)
         .exec((err, result)=>{
            if(err){
               ...
               reject(response.generate(true,'Failed to update password.',500,null));
            }else if(check.isEmpty(result)){
               logger.info('Failed to update password.','userController: updatePassword()',7);
               reject(response.generate(true,'Failed to update password.',404,null));
...
let resetPassword = (req, tokenDetails)=>{
   return new Promise((resolve, reject)=>{
      ...
      User.updateOne({'email':req.body.email}, updateQuery)
      .exec((err, result)=>{
         if(err){
            logger.error(err.message,'userController: resetPassword()',10);
            reject(response.generate(true,'Failed to reset user password.',500,null));
         }else if(check.isEmpty(result)){
            logger.info('Failed to reset user password.','userController: resetPassword()',7);
            reject(response.generate(true,'Failed to reset user password.',404,null));
         }else{
            setTimeout(()=>{
               mailer.sendPasswordResetEmail(tokenDetails.userDetails, updateQuery.resetToken);
            },2000);
            resolve(result);
         }
      })
...
let edit = (req)=>{
  return new Promise((resolve, reject)=>{
     ...
     User.update({'userId':req.params.userId}, updateQuery)
     .exec((err, result)=>{
        if(err){
         logger.error(err.message,'userController: edit()',10);
         reject(response.generate(true,'Failed to update user details.',500,null));

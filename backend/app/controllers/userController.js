...
let signUp = (req,res)=>{
   ...
   let validateParams = (req,res)=>{
      return new Promise((resolve, reject)=>{
         if(check.isEmpty(req.body.firstName) 
      ...
      if(!validate.checkEmail(req.body.email)){
         ...
         reject(response.generate(true,'Password length required 8.',400,null));
       }else if(!validate.checkUserName(req.body.userName)){
         
         reject(response.generate(true,'Username length required 6.',400,null));
       }else{ resolve(req); }
      });
   }
   ...
   .then((result)=>{
      ...
      res.send(response.generate(false,'User created successfully.',200,result));
    })
   .catch(err=>{
       res.send(err);
   });
...
let verifyUserEmail = (req,res)=>{
   ...
   .then((result)=>{
      ...
      delete result.createdOn;
      delete result.modifiedOn;
      res.send(response.generate(false,'User verfied successfully.',200,result));
   })
   .catch((err)=>{
      res.send(err);
   });
...
let logIn = (req,res)=>{
   let validateParams = ()=>{
      return new Promise((resolve, reject)=>{
         ...
         switch(loginType){
             case 'email': if(check.isEmpty(req.body.email) || check.isEmpty(req.body.password)){
                            reject(response.generate(true,'One or more parameter(s) is missing.',400,null));
                          } else if(!validate.checkEmail(req.body.email) || !validate.checkPassword(req.body.password)){
                            reject(response.generate(true,'Invalid email or password.',400,null));
                          }else{ resolve(req);}
                          break;

            case 'userName':if(check.isEmpty(req.body.userName) || check.isEmpty(req.body.password)){
                             reject(response.generate(true,'One or more parameter(s) is missing.',400,null));
                           }else if(!validate.checkUserName(req.body.userName) ||  !validate.checkPassword(req.body.password)){
                             reject(response.generate(true,'Invalid username or password.',400,null));
                           }else{ resolve(req);}
                           break;
            default: reject(response.generate(true,'One or more parameter(s) is missing.',400,null));
                     break;
         }
        });
   }
...
let changeUserPassword = (req,res)=>{
   ...
   let validateParams = ()=>{
      return new Promise((resolve, reject)=>{
         if(check.isEmpty(req.user.userId) 
         || check.isEmpty(req.body.password)
   ...
   .then((result)=>{
      res.send(response.generate(false,'Passwrod changed successfully.',200,null));
   })
   .catch((err)=>{
      res.send(err);
    });
...
let resetUserPassword = (req, res)=>{
   let validateParams = ()=>{
    ...
    .then((result)=>{
       res.send(response.generate(false,'Password reset email sent successfully.',200, null));
    })
    .catch((err)=>{
      res.send(err);
    });
...
let editUserDetails = (req,res )=>{
  ...
  .then(userLib.edit)
  .then((result)=>{
     res.send(response.generate(false,'User details updated successfully.',200,null));
  })
  .catch((err)=>{
   res.send(err);
 });
